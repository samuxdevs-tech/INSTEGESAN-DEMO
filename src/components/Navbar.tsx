...
export const Navbar: React.FC<NavbarProps> = ({ onGoHome, onGoMaster }) => {
    ...
    <header className="bg-slate-900/95 backdrop-blur border-b border-slate-800 text-white sticky top-0 z-40 no-print">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          {user?.rol === 'SUPER_ADMIN' && onGoMaster && (
            <button
              onClick={onGoMaster}
              className="px-3 py-1.5 bg-purple-950/90 hover:bg-purple-900 text-purple-300 border border-purple-700 rounded-xl text-xs font-black transition active:scale-95 shadow-sm"
              title="Ir al Panel Maestro de Control"
            >
              Panel Maestro
            </button>
          )}
